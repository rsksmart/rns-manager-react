...
const isValidTokenAmount = token => token >= 1;
...
class Bid extends Component {
  ...
  render() {
    const {
      strings, domain, bid, loading, viewMyCrypto,
    } = this.props;
    const {
      showMyCrypto, value, salt, isValid,
    } = this.state;
    ...
    return (
      <TabWithSearchComponent>
        ...
        >
          <Form.Group>
            <Form.Label>
              {strings.how_much_do_you_value}
              {' '}
              {domain}
              {'?'}
            </Form.Label>
            <InputGroup className="mb-3">
              <FormControl value={value} onChange={this.changeValue} type="number" className={!isValid ? 'is-invalid' : null} />
              <InputGroup.Append>
          ...
          <Button variant="link" disabled>{strings.advanced_options}</Button>
          <br />
          {
            viewMyCrypto
